...
function CalendarRow({ firstDay, lastDayInMonth, row, valueMonth, valueYear, currentDay, currentMonth, currentYear, onClick, max, min }: Props) {
      ...
      content.push(
        currentDay === i + 1 && valueMonth === currentMonth && valueYear === currentYear ? (
          <td key={"first" + i}>
            <button
              type="button"
              disabled={(min ? (currentMonth + 1 === parseInt(min.month) && currentYear === parseInt(min.year) - 543 ? parseInt(min.day) > i + (7 * row - firstDay) : false) : false) ? (min ? (currentMonth + 1 === parseInt(min.month) && currentYear === parseInt(min.year) - 543 ? parseInt(min?.day) > i + 1 : false) : false) : max ? (currentMonth + 1 === parseInt(max.month) && currentYear === parseInt(max.year) - 543 ? parseInt(max.day) < i + (7 * row - firstDay) : false) : false}
              onClick={() => onClick && onClick(i + 1)}
              className="w-7 h-7 bg-gray-500 text-white rounded-full flex justify-center items-center  disabled:cursor-not-allowed"
            >
              {i + 1}
            </button>
          </td>
          ...
          <td key={"first" + i}>
            <button
              type="button"
              disabled={(min ? (currentMonth + 1 === parseInt(min.month) && currentYear === parseInt(min.year) - 543 ? parseInt(min.day) > i + (7 * row - firstDay) : false) : false) ? (min ? (currentMonth + 1 === parseInt(min.month) && currentYear === parseInt(min.year) - 543 ? parseInt(min?.day) > i + 1 : false) : false) : max ? (currentMonth + 1 === parseInt(max.month) && currentYear === parseInt(max.year) - 543 ? parseInt(max?.day) < i + 1 : false) : false}
  ...
  for (let i = 1; i <= 7; i++) {
    if (i + (7 * row - firstDay) <= lastDayInMonth) {
      content.push(
        isSelected(i + (7 * row - firstDay)) ? (
          <td key={"other" + i}>
            <button
              type="button"
              disabled={(min ? (currentMonth + 1 === parseInt(min.month) && currentYear === parseInt(min.year) - 543 ? parseInt(min.day) > i + (7 * row - firstDay) : false) : false) ? (min ? (currentMonth + 1 === parseInt(min.month) && currentYear === parseInt(min.year) - 543 ? parseInt(min.day) > i + (7 * row - firstDay) : false) : false) : max ? (currentMonth + 1 === parseInt(max.month) && currentYear === parseInt(max.year) - 543 ? parseInt(max.day) < i + (7 * row - firstDay) : false) : false}
