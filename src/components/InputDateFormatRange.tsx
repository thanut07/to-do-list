...
function InputDateFormat({ minDate, maxDate, title = "", placeholder = "วว/ดด/ปปปป", disabled = false, name = "", id = "", className = "", onChange, value = "", stroke = "", classCalendar = "", titleElement, error, touched, ...props }: Props) {
        ...
        <div ref={ref} className="w-full relative ">
            ...
            <div className="relative z-0">
                <input
                    ...
                    value={defaultValue}
                    className={`
                        block w-full pl-8 py-2 rounded-md border focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all duration-150 
                        ${className ? className : " "}
                ...
                <button
                    type="button"
                    disabled={disabled}
                    className={`flex absolute inset-y-0 right-2 -top-[2px] z-30 items-center cursor-pointer mt-1`}
                    onClick={() => {
                        setOpenCalendar(!isOpenCalendar);
                    }}
                >
